...
const RoomListView = () => {
    ...
    return (
        <Layout title={'방목록'} buttonText={'방 만들기'} _onClick={createRoom}>
            ...
            <div className="flex items-center w-[100%]  px-[100px] text-[16px] text-[#444444] border-b-[#e0e0e0] border-b-2 pb-[14px]">
            ...
            <div className="w-[100vw] px-[100px]">
                ...
                <ArtistProvider role={role}>
                    {roomList
                        .filter((room) => room.artist_id === userInfo.artistNo || userInfo.no)
                        .map((room, idx) => {
                            return (
                                <Room
                                    room={room}
            ...
            {isOpenRoomCreate && (
                <CreateRoom
                    setOnModal={() => setIsOpenRoomCreate(false)}
...
const Room = ({ room, endRoomApi, setCurrentRoom, id, bgColor, eventList, navigate, dispatch }) => {
    ...
    return (
        <div className={`flex items-center h-[70px] ${bgColor} w-[100%] text-[19px]`} key={id}>
            <div className="w-[30vw]">{room.room_name}</div>

            <div className="font-bold w-[20vw]">{room.artist_name}</div>

            <div className="w-[30vw]">{room.fan_name}</div>
            ...
            <Button _onClick={() => roomEnd(room)} coverWidth={'w-[10vw]'}>
                <div className={'w-[25px] h-[25px]'}>
