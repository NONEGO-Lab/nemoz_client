...
const CreateRoom = ({ setOnModal, getEventListApi, eventList }) => {
    ...
    const [targetArtistIds, setTargetArtistIds] = useState(
        eventList.map((e) => e.target_artist_ids)[0]?.map((artist) => artist.name)
    );
    ...
    const valueMaker = useCallback(
        (array) =>
            array.map(({ no: value, name: label, ...rest }) => ({
                label,
                value,
                ...rest,
            })),
        ...
        <ModalFrame setOnModal={setOnModal} style={modalStyle}>
            <div className={'p-[60px]'}>
                ...
                <div className="flex justify-between mb-[56.5px]">
                    <div className="text-[24px] font-medium text-[#444]">방 만들기</div>
                    <div onClick={() => setOnModal(false)} className={'w-[20px] flex items-center cursor-pointer'}>
                ...
                <form>
                    {/*Select Event*/}
                    <div className={`flex items-center justify-between  h-[18.5px]`}>
                        <label
                            htmlFor={'roomTitle'}
                        ...
                        <select
                            ...
                            onChange={(e) => {
                                ...
                                setTargetArtistIds(
                                    eventList.find((e) => e.event_name === target)?.target_artist_ids.map((a) => a.name)
                                );
                                setTargetArtistFullInfo(
                                    eventList.find((e) => e.event_name === target).target_artist_ids
                                );
                    ...
                    <Select
                        ...
                        border={'none'}
                        style={
                            'overflow-hidden whitespace-nowrap appearance-none bg-arrow-down bg-no-repeat bg-15 bg-right pr-[15px] '
                        }
                    />
                    <div className={'w-full h-[1.5px] bg-[#c7c7c7] mt-[28px] mb-[44px] mx-0'} />
