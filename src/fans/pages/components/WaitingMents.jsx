...
const WaitingMents = ({ isCallFinished, myWaitInfo, fan_name, eventTitle, isReadyTest }) => {
    ...
    if (isCallTested) {
        console.log('isCallTested : ', isCallTested);
        console.log('isReadyTest : ', isReadyTest);
        console.log('isCallFinished : ', isCallFinished);
        return (
            <>
                ...
                {!isReadyTest ? (
                    <div className={'text-[20px]'}>
                        <b>{`${fan_name}님`}</b>의 테스트 순서 대기중입니다.
                        <span className={'text-[#00cace]'}>
                            {' '}
                            예상 테스트 대기 시간은 {secondsToMins(myWaitInfo.waiting?.wait_seconds)}분입니다.
                        </span>
                    </div>
                ) : (
                    <div className={'text-[20px]'}>
                        <b>{`${fan_name}님`}</b>의 테스트 순서입니다.
